...
namespace ExpertMultimedia {
	public class Table {
		...
		public string sFieldDelim { get { return char.ToString(cFieldDelim); } }
		public string sTextDelim { get { return char.ToString(cTextDelim); } }
		public char cFieldDelim=',';//public static string sFieldDelimiter=",";
		public char cTextDelim='"';//public static string sTextDelimiter="\"";
		...
		private int Maximum {
			...
			set {
				try {
					...
					Console.Error.WriteLine("Error setting Table line array length:");
		...
		public int GetOrCreateColumnNumber(string sColumnTitle) {
			...
			if (teTitles==null) {
				...
				teTitles.Columns=1;
				iReturn=0;
				Console.WriteLine("Warning: Had to create first column header \""+sColumnTitle+"\"");
			}
			else {
				iReturn=teTitles.IndexOfExactValue(sColumnTitle);
				if (iReturn<0) {
					Console.WriteLine("Warning: Had to create additional column header \""+sColumnTitle+"\" at column "+teTitles.Columns);
		...
		public static string CSVFieldToRawField(string sCSVField, string cFieldDelimX, char cTextDelimX) {
			if (sCSVField.StartsWith(cFieldDelimX)) {
				if (sCSVField.Length>2) sCSVField=sCSVField.Substring(1,sCSVField.Length-2);
				//else load as literal
			}
			sCSVField=sCSVField.Replace(char.ToString(cTextDelimX)+char.ToString(cTextDelimX),char.ToString(cTextDelimX));
			for (int iNow=0; iNow<sarrEscapeLiteral.Length; iNow++) {
			...
			if (sLine!=null) {
				if (sLine!="") {
					while (iChar<=sLine.Length) {//intentionally <=
						if ( iChar==sLine.Length || sLine[iChar]==cFieldDelimX ) {
							iEnderNow=iChar;
							iFound++;
						}
						else if (sLine[iChar]==cTextDelimX) bInQuotes=!bInQuotes;
						iChar++;
					}
				}
				else iFound=1;
		...
		public string RowToCSVLine(int iRow, bool bReplaceNewLineWithTabInsteadOfHTMLBrWithMarkerProperty) {
			...
			if (iRow<iRows&&iRow>-1) {
				if (tearr[iRow]!=null) sReturn=tearr[iRow].ToCSVLine(cFieldDelim, cTextDelim, bReplaceNewLineWithTabInsteadOfHTMLBrWithMarkerProperty);
			}
		...
		public bool Save(string sFile, bool bReplaceNewLineWithTabInsteadOfHTMLBrWithMarkerProperty) {
			...
			try {
				...
				if (bFirstRowContainsData) fsDest.WriteLine(teTitles.ToCSVLine(cFieldDelim,cTextDelim,bReplaceNewLineWithTabInsteadOfHTMLBrWithMarkerProperty));
...
//////////////////////////////////// TableEntry /////////////////////////////////////////////
	...
	public class TableEntry {
		...
		public string ToCSVLine(char cFieldDelimX, char cTextDelimX, bool bReplaceNewLineWithTabInsteadOfHTMLBrWithMarkerProperty) {
			return Table.RowToCSVLine(fieldarr, cFieldDelimX, cTextDelimX, iCols, bReplaceNewLineWithTabInsteadOfHTMLBrWithMarkerProperty);
		}
